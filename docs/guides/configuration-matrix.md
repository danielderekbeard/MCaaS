...
|-------------|--------------|------------------|-------|
| Auth method | `existingSecret` | Change `global.postgresql.auth.existingSecret` to point to a different K8s secret | Must match the secret created by `create_secrets()` in deploy.py |
| Secret name | `mcaas-postgresql-secret` | Update in both this file AND `create_secrets()` in deploy.py | Secret must exist in `managed-it` namespace |
...
| Namespace | `managed-it` | Change in `deploy/namespaces.yaml` and deploy.py | All consumers must be updated |

**Making it unique**: Change the Helm release name prefix in `deploy.py` (default `mcaas-postgresql`), update all downstream references to the service FQDN.
...
| Secret name | `mcaas-opensearch-secret` | Update `extraEnvs[0].valueFrom.secretKeyRef.name` | Also update `create_secrets()` in deploy.py |
...
| Namespace | `security-ops` | Change in `deploy/namespaces.yaml` and deploy.py | |
...
| Config Item | Current Value | How to Customize | Notes |
|-------------|--------------|------------------|-------|
| Deployment method | kustomize (git clone) | Create a kustomize overlay in `deploy/wazuh-overlay/` | deploy.py clones `wazuh-kubernetes` repo with `--no-checkout` |
| Overlay used | `local-env` | Change `WAZUH_OVERLAY` in deploy.py or create custom overlay | The upstream overlay includes its own indexer |
...
| StorageClass | `wazuh-storage` (created by deploy.py) | Change the StorageClass manifest in `deploy_wazuh()` | Uses rancher.io/local-path provisioner with WaitForFirstConsumer |
| Namespace | `wazuh` | Change in `deploy/namespaces.yaml` and deploy.py | |
...
| Namespace | `security-ops` | Change in `deploy/namespaces.yaml` and deploy.py | |
| Release name | `mcaas-shuffle` | Change the `helm upgrade --install` release name in deploy.py | |
...
| Namespace | `managed-it` | Change in `deploy/namespaces.yaml` and deploy.py | |
| Release name | `zammad` | Change the helm upgrade release name in deploy.py | |
...
| Namespace | `grc` | Change in `deploy/namespaces.yaml` and deploy.py | |
| Release name | `ciso-assistant` | Change the helm upgrade release name in deploy.py | |
...
4. Update `deploy.py` (namespace arguments in `helm upgrade --install` and `kubectl` commands)
...
6. `deploy.py` → `create_secrets()` function
