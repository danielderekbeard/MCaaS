...
the Kubernetes stack deployed by the MCaaS scripts/deploy.py script.
"""
...
def shutdown():
    ...
    logging.info(
        "Graceful shutdown complete. All MCaaS compute resources are suspended."
    )
...
def poweron():
    ...
    if not STATE_FILE.exists():
        ...
        logging.error(
            "If you restarted your machine, you can also just re-run scripts/deploy.py."
        )
        return
    ...
    for ns, resources in state.items():
        for resource_name, original_replicas in resources.items():
            logging.info(
                f"Restoring {resource_name} in {ns} to {original_replicas} replicas..."
            )
            try:
                run_command(
                    [
    ...
    logging.info(
        "Power on initiated. Wait a few minutes for all pods to reach a 'Ready' state."
    )


def main():
    parser = argparse.ArgumentParser(
        description="Gracefully shutdown or power on the MCaaS stack."
    )
    parser.add_argument(
        "action", choices=["shutdown", "poweron"], help="Action to perform"
    )
    ...
    try:
        ...
        logging.error(
            "kubectl cannot authenticate to the cluster. Check your kubeconfig."
        )
        return
